...
const StatCard = ({ icon: Icon, value, label }: { icon: any; value: string; label: string }) => (
  <motion.div 
    ...
    className="border border-border bg-card/50 p-10 flex flex-col items-center gap-4 transition-colors hover:border-primary hover:bg-card"
  >
...
const CoreCard = ({ icon: Icon, title, desc }: { icon: any; title: string; desc: string }) => (
  <motion.div 
    ...
    className="lg:flex-1 border border-border bg-card/30 p-12 group transition-colors hover:border-primary hover:bg-card/80"
  >
    <Icon size={48} className="text-primary mb-10 group-hover:scale-110 group-hover:rotate-6 transition-transform duration-500" />
    <h4 className="text-3xl font-heading font-black text-foreground mb-6 uppercase tracking-tighter italic">{title}</h4>
...
export const AboutUs: React.FC = () => {
  return (
    <section id="about" className="py-48 relative bg-background text-foreground px-8 md:px-24">
      <div className="container mx-auto relative z-10">
        ...
        >
          ...
          <motion.div variants={fadeInUp} className="flex-1">
            ...
            <h3 className="text-6xl md:text-9xl font-heading font-black text-foreground uppercase italic tracking-tighter leading-none mb-12">
              Engineering <br /> <span className="text-foreground/20">Legacy</span>
            </h3>
          ...
          <motion.div 
            variants={staggerContainer}
            className="flex-1 w-full flex flex-col gap-px bg-border border border-border"
          >
