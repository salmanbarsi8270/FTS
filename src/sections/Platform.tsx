...
const PlatformCard = ({ icon: Icon, title, desc }: { icon: any; title: string; desc: string }) => (
  <motion.div 
    ...
    className="relative group p-10 lg:p-12 bg-neutral-900/60 border border-white/10 flex flex-col items-start gap-8 transition-all duration-500 rounded-3xl hover:bg-neutral-800 hover:border-primary/50 shadow-xl overflow-hidden"
  >
    {/* Glow Aura */}
    ...
    <div className="w-16 h-16 rounded-2xl border border-white/10 bg-black/50 flex items-center justify-center group-hover:bg-primary group-hover:border-primary group-hover:shadow-[0_0_20px_rgba(79,70,229,0.4)] transition-all duration-500 relative z-10">
      <Icon size={32} className="text-primary group-hover:text-white transition-colors" />
    </div>
    <div className="flex flex-col gap-4 relative z-10">
      <h3 className="text-3xl font-heading font-black text-white uppercase tracking-tighter group-hover:text-primary transition-colors italic">{title}</h3>
...
export const Platform: React.FC = () => {
  ...
  return (
    <section id="platform" className="py-48 relative overflow-hidden bg-black text-white">
      <div className="container mx-auto px-8 relative z-10">
        ...
        >
          <motion.div variants={fadeInUp}>
            ...
            <h3 className="text-6xl md:text-9xl font-heading font-black text-white uppercase italic tracking-tighter leading-none drop-shadow-lg">
              Platform
            </h3>
          </motion.div>
        ...
        >
          ...
          <motion.div variants={fadeInUp} className="relative bg-neutral-900/80 backdrop-blur-2xl border border-white/10 rounded-3xl p-12 lg:p-16 flex flex-col lg:flex-row items-center justify-between gap-12 text-white overflow-hidden shadow-2xl">
            ...
            <div className="flex flex-col gap-4 text-center lg:text-left relative z-10 w-full lg:w-1/2">
              <h4 className="text-4xl md:text-6xl font-heading font-black uppercase tracking-tighter italic text-white drop-shadow-md">ZATCA Phase 2</h4>
              <p className="text-white/60 text-xl font-medium">Seamless integration with Saudi Arabian e-invoicing portals. Compliant. Secure. Native.</p>
            </div>
            
            <motion.button 
              whileHover={springHover.scale ? { scale: 1.05 } : undefined}
              whileTap={{ scale: 0.95 }}
              className="relative z-10 px-10 py-5 bg-primary/20 backdrop-blur-xl border border-primary/50 text-white font-black text-sm uppercase tracking-widest rounded-full hover:bg-primary/40 hover:border-primary/80 transition-all shadow-[0_8px_32px_rgba(79,70,229,0.3)] whitespace-nowrap"
            >
              Audit Architecture
            </motion.button>
          </motion.div>
